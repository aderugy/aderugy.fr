...
-- Done row by row rather than as one insert...select because the new task's id
-- has to be linked back to the block it came from, which `returning` cannot
-- carry across a set-based insert.
do $$
declare
  b        record;
  minutes  int;
